...
\item (c)$b\in \Gamma(X_f,\mathcal{O}_{X_f}
)$ induce that $b|_{U_i\cap X_f} \in \Gamma(U_i \cap X_f,\mathcal{O}_{U_i}\cap X_f)=B_{i_f}$. Here we assume $U_i=Spec \  B_i$ and hence $\exists l_i$ s.t $b|_{U_i\cap X_f}=a_i/f^{l_i}$. Thus let $l$ be the max $l_i$ and let $c=bf^l$, then $\exists c_i \in B_i$ s.t $c|U_i \cap X_f=c_i|U_i \cap X_f$. 
